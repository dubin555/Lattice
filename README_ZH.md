...
**同步阻塞设计**：LangGraph 客户端有意采用同步阻塞调用，这是合理的设计选择：

- **外层 FastAPI 是异步的** — 多个请求可以并发处理，不会互相阻塞
- **执行引擎 Ray 是异步的** — 任务在分布式 Worker 上并行执行
- **LangGraph 调用点同步等待** — 符合正常的函数调用语义，代码直观简洁

如果改成异步回调模式，LangGraph 的工作流代码会变得复杂很多（需要管理状态机、回调注册、结果拼接等），而收益不大 — 因为真正的并发已经在上下两层解决了。
