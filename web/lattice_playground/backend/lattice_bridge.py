"""
Lattice Client 桥接模块 - 完整版
用于Node.js后端调用Python Lattice Client
"""
...
# 使用客户端模块
from lattice import LatticeClient as MaClient, task, get_task_metadata
import inspect
import importlib
...
def parse_custom_function(code):
    ...
    try:
        ...
        for name, obj in inspect.getmembers(module):
            if hasattr(obj, '_lattice_task_metadata'):
                metadata = obj._lattice_task_metadata
        ...
        return {"error": f"Import failed: {e}. Please use 'from lattice import task'", "traceback": traceback.format_exc()}
...
def create_lattice_workflow(workflow_id, server_url="http://localhost:8000"):
    """
    创建 Lattice 工作流（已废弃）
    ...
    return {
        "success": True,
        "latticeWorkflowId": "will-be-created-on-run"
    }
...
def build_and_run_workflow(workflow_id, nodes, edges):
    ...
    try:
        ...
        print(f"[DEBUG] Lattice 工作流已创建: {workflow.workflow_id}", file=sys.stderr)
        ...
        for node in nodes:
            ...
            if category == "builtin":
                return {"success": False, "error": "内置任务暂未迁移到 v2，请使用自定义任务"}
...
def main():
    ...
    try:
        if action == 'get_builtin_tasks':
            ...
            result = create_lattice_workflow(
