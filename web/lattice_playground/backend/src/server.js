...
function callPython(action, params = {}) {
  return new Promise((resolve, reject) => {
    const bridgePath = path.join(__dirname, '../lattice_bridge.py');
...
app.post('/api/workflows', async (req, res) => {
  try {
    ...
    if (!result.success) {
      console.error('❌ 创建 Lattice 工作流失败:', result.error);
    ...
    workflows.set(workflowId, {
      id: workflowId,
      name,
      latticeWorkflowId: result.latticeWorkflowId,
      nodes: [],
      edges: [],
    ...
    console.log(`✅ 工作流创建成功 (Lattice ID: ${result.latticeWorkflowId})`);
    res.json({ 
      workflowId, 
      name,
      latticeWorkflowId: result.latticeWorkflowId
...
server.listen(PORT, () => {
  console.log('\n' + '='.repeat(60));
  console.log('  🚀 Lattice Playground Backend Server');
