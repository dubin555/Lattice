# Lattice Board 前后端集成指南

本指南说明了 Lattice Board 前端如何支持 **Mock 模式**和 **API 模式**的双重运行方式。
...
在 `web/lattice_board/` 目录下创建 `.env` 文件（参考 `.env.example`）：
